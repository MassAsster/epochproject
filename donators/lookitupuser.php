...
$idname=$_POST['idlookup'];
$idname=mysql_real_escape_string($idname);
if($multicharactersupport == 1 ) {
$slot=$_POST['slot'];
$slot=mysql_real_escape_string($slot);
$result44 = mysql_query("SELECT COUNT(*) FROM $chartablename WHERE PlayerUID='$idname' AND Alive='1' AND Slot='$slot'");
}
else {
$result44 = mysql_query("SELECT COUNT(*) FROM $chartablename WHERE PlayerUID='$idname' AND Alive='1'");
}
if (!$result44) {
    die(mysql_error());
    }
if (mysql_result($result44, 0, 0) == 0) {
Echo "<center><font color=red>This player does not have a character that is currently alive.<P><center><input type='button' value='Back' onclick='goBack()'>";
}
else {
	}
$result = mysql_query("SELECT * FROM $chartablename WHERE PlayerUID='$idname' AND Alive='1'");
...
echo "<center><input type='button' value='Back' onclick='goBack()'>";
}
