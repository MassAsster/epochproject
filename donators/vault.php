...
<P>
<TABLE BORDER=1><th bgcolor="#003399"> <center><h3><font color=#ffffff>Change Vault Code</font> </h3><TR><TD>
...
if($allowvaultchange == 0) {
		echo "Server Administrator Has Disabled This Feature";
		exit();
}
else {
}
	$whois="<form enctype=multipart/form-data action=vault2.php method=POST><input type=hidden name=player value=$guid ><input type=hidden name=cash value=$tokens ><input type=image src=images/doit.png></form>";
if($tokens >= $coinsforvault) {
echo "<center>Use $coinsforvault Token(s) from your stash of $tokens <P> to change a vault code<P>$whois";
}
else {
