...
$result33 = mysql_query("SELECT COUNT(*) FROM `authorize` WHERE serverop='$id'");
if (!$result33) {
    die(mysql_error());
}
if (mysql_result($result33, 0, 0) > 0) {
 echo "ERROR: This Pin not valid";
...
mysql_query("INSERT INTO `donatorlog` (`date`, `user`, `action`) VALUES
 ('$date', '$user', 'PIN Setup: added $id ');");
}
mysql_query("UPDATE `authorize` SET serverop='$id' WHERE username='$user' ");
